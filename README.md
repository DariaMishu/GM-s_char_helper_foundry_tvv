...
- **Два режима скейлинга — КО и уровень**. Оба режима автоматически
  ...
  - HP = базовый HP режима × множитель класса (`hp_multiplier`,
    d6=0.80 / d8=1.00 / d10=1.15 / d12=1.30) × множитель размера из `sizes.json`
    (Tiny=0.50, Small=0.75, Medium=1.00, Large=1.25, Huge=1.50, Gargantuan=2.00 —
    выведены из средних монстровых хитдайсов d4…d20).
...
- **Размер существа** — выбирается из стандартной сетки D&D (Tiny…Gargantuan)
  и автоматически ставит `system.traits.size`, размер токена в клетках
  (`prototypeToken.width`/`height`: tiny 0.5×0.5, sm/med 1×1, lg 2×2, huge 3×3, grg 4×4),
  а также влияет на рекомендуемый HP через `hp_multiplier` (большие
  существа пропорционально живучее маленьких).
- **Режим «Монстр» — разумные дефолты**. При включённом чекбоксе «Монстр»
  класс по умолчанию — «Иное / без класса» (нейтральные множители), отношение
  к игрокам — «Враждебный». Для PC по умолчанию — «Варвар» и «Нейтральный».
- **Дополнительные виды передвижения** — в раскрывающемся блоке можно задать
  скорость полёта (`fly`), лазанья (`climb`), плавания (`swim`) и ройки (`burrow`),
  а также флаг «Парит» (`hover`). По умолчанию все поля пустые —
  в JSON пробрасывается `null`, чтобы Foundry работала со стандартной логикой
  (лазанье/плавание в ½ скорости). Заполненные поля попадают в
  `system.attributes.movement.{fly,climb,swim,burrow,hover}`.
...
| Полёт / лазанье / плавание / ройка | `system.attributes.movement.{fly,climb,swim,burrow}` |
| Парит (hover) | `system.attributes.movement.hover` |
